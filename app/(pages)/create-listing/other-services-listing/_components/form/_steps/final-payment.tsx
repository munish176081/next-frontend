...
const FinalPayment = () => {
  ...
  const { data, isLoading: isLoadingProducts } = useListingProducts("other");
  const listingProducts = data?.products ?? [];

  const { data: adsData = [] } = useListingAds("other");
  ...
  return (
    <div className="mt-6 pb-40">
      <RequireUser
        ...
        successRedirect={`${Routes.public.createListing.otherServicesListing}?step=${
          stepIdx + 1
        }`}
      >
        <>
          <label className="text-h5">
            ...
            {isLoadingProducts ? (
              ...
              >
                ...
                >
                  <SelectValue placeholder="Select other listing" />
                </SelectTrigger>
                <SelectContent>
