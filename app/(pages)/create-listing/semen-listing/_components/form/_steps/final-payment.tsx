...
const FinalPayment = () => {
  ...
  const { data, isLoading: isLoadingProducts } = useListingProducts("semen");
  const listingProducts = data?.products ?? [];

  const { data: adsData = [] } = useListingAds("semen");
  ...
  return (
    <div className="mt-6 pb-40">
      <RequireUser
        ...
        successRedirect={`${Routes.public.createListing.semenListing}?step=${
          stepIdx + 1
        }`}
      >
        <>
          <label className="text-h5">
            ...
            {isLoadingProducts ? (
              ...
              >
                ...
                >
                  <SelectValue placeholder="Select semen listing" />
                </SelectTrigger>
                <SelectContent>
