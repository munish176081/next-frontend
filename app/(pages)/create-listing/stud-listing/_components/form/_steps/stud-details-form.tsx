...
const MAX_STUD_PRICE = 100_000_000;

export const studDetailsSchema = z.object({
  ...
  provenLittersVideoUrls: z.array(videoUrlSchema).optional().default([]),
  videoUrls: z.array(videoUrlSchema).optional().default([]),
});

export type StudDetailsType = z.infer<typeof studDetailsSchema>;

export const StudDetailsForm = () => {
  const {
    stepIdx,
    totalSteps,
    formData,
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<StudDetailsType>({
    defaultValues: {
      title: (formData as StudDetailsType).title,
      breed: (formData as StudDetailsType).breed,
      dateOfBirth: (formData as StudDetailsType).dateOfBirth,
      healthCertificatesText: (formData as StudDetailsType)
        .healthCertificatesText,
      healthCertificatesImages: (formData as StudDetailsType)
        .healthCertificatesImages,
      serviceFee: (formData as StudDetailsType).serviceFee,
      availibilityDate: (formData as StudDetailsType).availibilityDate,
      dogName: (formData as StudDetailsType).dogName,
      ankcNumber: (formData as StudDetailsType).ankcNumber,
      images: (formData as StudDetailsType).images,
      videoUrls: (formData as StudDetailsType).videoUrls,
      provenLittersImages: (formData as StudDetailsType).provenLittersImages,
      provenLittersVideoUrls: (formData as StudDetailsType)
        .provenLittersVideoUrls,
    },
    mode: "onChange",
    resolver: zodResolver(studDetailsSchema),
  });

  const values = watch();

  function handleFormSubmit(data: StudDetailsType) {
    goToNextStep(data);
  }
    ...
    >
      ...
      <Controller
        ...
        render={({ field }) => (
          <>
            <FileUploader
              label="Upload stud or bitch images"
              accept="image/*"
              multiple
              onChange={(files) => {
                field.onChange([...(field.value ?? []), ...Array.from(files)]);
      ...
      <Controller
        ...
        render={({ field }) => (
          <>
            <Input
              type="text"
              label="Enter stud or bitch video URLs (comma separated)"
