...
const FinalPayment = () => {
  ...
  const { data, isLoading: isLoadingProducts } = useListingProducts("future");
  const listingProducts = data?.products ?? [];

  const { data: adsData = [] } = useListingAds("future");
  ...
  return (
    <div className="mt-6 pb-40">
      <RequireUser
        ...
        successRedirect={`${Routes.public.createListing.futureListing}?step=${
          stepIdx + 1
        }`}
      >
        <>
          <label className="text-h5">
            ...
            {isLoadingProducts ? (
              ...
              >
                ...
                >
                  <SelectValue placeholder="Select future listing" />
                </SelectTrigger>
                <SelectContent>
