...
const FinalPayment = () => {
  ...
  const { data, isLoading: isLoadingProducts } =
    useListingProducts("wanted_puppy");
  const listingProducts = data?.products ?? [];

  const { data: adsData = [] } = useListingAds("wanted_puppy");
  ...
  return (
    <div className="mt-6 pb-40">
      <RequireUser
        ...
        successRedirect={`${Routes.public.createListing.wantedPuppyListing}?step=${
          stepIdx + 1
        }`}
      >
        <>
          <label className="text-h5">
            ...
            {isLoadingProducts ? (
              ...
              >
                ...
                >
                  <SelectValue placeholder="Select wanted puppy listing" />
                </SelectTrigger>
                <SelectContent>
