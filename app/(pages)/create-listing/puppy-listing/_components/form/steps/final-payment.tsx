...
const FinalPayment = () => {
  ...
  const { data, isLoading: isLoadingProducts } = useListingProducts("puppy");
  const listingProducts = data?.products ?? [];

  const { data: adsData = [] } = useListingAds("puppy");
  ...
  return (
    <div className="mt-6 pb-40">
      <RequireUser
        ...
        successRedirect={`${Routes.public.createListing.puppyListing}?step=${
          stepIdx + 1
        }`}
      >
        <>
