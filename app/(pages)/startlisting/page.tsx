...
function Startlisting() {
  ...
  const handleListingSelect = (listingEnum: string) => {
    // Automatically move forward when a listing type is selected
    const shortCode = getShortCodeFromId(listingEnum);
    ...
    <section className="container relative overflow-hidden p-8 rounded-max bg-white max-md:p-4 max-md:rounded-40">
      ...
      <div className="grid grid-cols-2 gap-6 relative  mt-8 max-md:flex-col max-md:gap-4 max-md:mt-4 max-md:grid-cols-1">
        {LISTING_TYPES.map((listing, index) => (
          ...
          >
            <div className="p-6 border border-black/20 rounded-40 bg-white gap-2 flex flex-col max-md:p-4 max-md:rounded-[20px] items-center transition-all duration-300 hover:border-black/40">
